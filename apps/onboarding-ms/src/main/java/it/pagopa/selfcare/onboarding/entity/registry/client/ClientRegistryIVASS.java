...
public abstract class ClientRegistryIVASS extends BaseRegistryManager<InsuranceCompanyResource> {
    ...
    private final IvassControllerApi client;

    protected ClientRegistryIVASS(Onboarding onboarding, IvassControllerApi client) {
        super(onboarding);
        this.client = client;
    }

    public InsuranceCompanyResource retrieveInstitution() {
        return client.searchInsurancesUsingGET(onboarding.getInstitution().getOriginId())
