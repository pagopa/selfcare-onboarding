...
public class OnboardingUtils {
  ...
  private static final String ADDITIONAL_INFORMATION_REQUIRED =
      "Additional Information is required when institutionType is GSP and productId is pagopa";
  private static final String OTHER_NOTE_REQUIRED =
      "Other Note is required when other boolean are false";
  private static final String BILLING_OR_RECIPIENT_CODE_REQUIRED =
      "Billing and/or recipient code are required";
  private static final String ONBOARDING_NOT_ALLOWED_ERROR_MESSAGE_NOT_DELEGABLE =
      "Institution with external id '%s' is not allowed to onboard '%s' product because it is not delegable";
  private static final String PARENT_TAX_CODE_IS_INVALID =
      "The tax code of the parent entity of the request does not match the tax code of the parent entity retrieved by IPA";
  private static final String TAX_CODE_INVOICING_IS_INVALID =
      "The tax code invoicing of the request does not match any tax code of institutions' hierarchy";

  public Uni<Onboarding> customValidationOnboardingData(
      Onboarding onboarding, Product product, ProxyResource proxyResource) {
    if (isUOWithSfe(onboarding)) {
      return checkParentTaxCode(
              onboarding, ((UOResource) proxyResource.getResource()).getCodiceFiscaleEnte())
          .onItem()
          .transformToUni(o -> checkTaxCodeInvoicing(onboarding, product));
    } else {
  ...
  private Uni<Void> checkRecipientCode(Onboarding onboarding, ProxyResource proxyResource) {
    if (isInvoiceablePA(onboarding)) {
      ...
      return getUoFromRecipientCode(recipientCode)
          .onItem()
          .transformToUni(
              uoResource -> validationRecipientCode(onboarding, proxyResource, uoResource))
          .onItem()
          .transformToUni(
              customError -> {
                if (Objects.nonNull(customError)) {
                  return Uni.createFrom()
                      .failure(new InvalidRequestException(customError.getMessage()));
                }
  ...
  private Uni<CustomError> validationRecipientCode(
      Onboarding onboarding, ProxyResource proxyResource, UOResource uoResource) {

    switch ((onboarding.getInstitution().getSubunitType() != null)
        ? onboarding.getInstitution().getSubunitType()
        : EC) {
      case AOO -> {
        return getValidationRecipientCodeError(
            ((AOOResource) proxyResource.getResource()).getCodiceIpa(), uoResource);
      }
      case UO -> {
        return getValidationRecipientCodeError(
            ((UOResource) proxyResource.getResource()).getCodiceIpa(), uoResource);
      }
      default -> {
        return getValidationRecipientCodeError(
            onboarding.getInstitution().getOriginId(), uoResource);
      }
    }
  }

  /**
   * Validate fields of onboarding in case of PRV or SCP If digitalAddress or description does not
   * match proxy data, an exception is thrown
   */
                  ...
                  return Uni.createFrom()
                      .failure(
                          new InvalidRequestException(
                              "Field digitalAddress or description are not valid"));
                }
  ...
  public Uni<UOResource> getUoFromRecipientCode(String recipientCode) {
    return uoApi
        .findByUnicodeUsingGET1(recipientCode, null)
        .onFailure(WebApplicationException.class)
        .recoverWithUni(
            ex ->
                ((WebApplicationException) ex).getResponse().getStatus() == 404
                    ? Uni.createFrom()
                        .failure(
                            new ResourceNotFoundException(
                                String.format(UO_NOT_FOUND.getMessage(), recipientCode)))
                    : Uni.createFrom().failure(ex));
  }

  public Uni<CustomError> getValidationRecipientCodeError(
      String originIdEC, UOResource uoResource) {
  ...
  private Uni<Onboarding> checkTaxCodeInvoicing(Onboarding onboarding, Product product) {
    ...
    return uoApi
        .findAllUsingGET1(null, null, onboarding.getBilling().getTaxCodeInvoicing())
        .flatMap(
            uosResource -> {
              /* if parent tax code is not into hierarchy, throw an exception */
              if (Objects.nonNull(uosResource)
                  && Objects.nonNull(uosResource.getItems())
                  && uosResource.getItems().stream()
                      .anyMatch(
                          uoResource ->
                              !uoResource
                ...
                return Uni.createFrom()
                    .failure(new InvalidRequestException(TAX_CODE_INVOICING_IS_INVALID));
              }
  ...
  private Uni<Onboarding> additionalChecksForProduct(Onboarding onboarding, Product product) {
    ...
    if (InstitutionType.PA.equals(onboarding.getInstitution().getInstitutionType())
      ...
      if (Objects.isNull(onboarding.getBilling())
          || Objects.isNull(onboarding.getBilling().getRecipientCode()))
        return Uni.createFrom()
            .failure(new InvalidRequestException(BILLING_OR_RECIPIENT_CODE_REQUIRED));
    }
    if (InstitutionType.GSP == onboarding.getInstitution().getInstitutionType()
      ...
      if (Objects.isNull(onboarding.getAdditionalInformations())) {
        return Uni.createFrom()
            .failure(new InvalidRequestException(ADDITIONAL_INFORMATION_REQUIRED));
  ...
  public static class ProxyResource<T> {
    ...
    public InstitutionPaSubunitType getType() {
      return type;
    }

    public void setType(InstitutionPaSubunitType type) {
      this.type = type;
    }
  ...
  private boolean originPDNDInfocamere(
      Onboarding onboarding, PDNDBusinessResource pdndBusinessResource) {
    return onboarding
            .getInstitution()
            .getDigitalAddress()
            .equals(pdndBusinessResource.getDigitalAddress())
        && onboarding
            .getInstitution()
            .getDescription()
            .equals(pdndBusinessResource.getBusinessName());
  }
}
