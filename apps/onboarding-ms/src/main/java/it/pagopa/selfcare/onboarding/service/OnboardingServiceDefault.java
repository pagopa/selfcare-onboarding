...
public class OnboardingServiceDefault implements OnboardingService {
        ...
        .transformToUni(
            product ->
                getRegistryResource(onboarding)
                    ...
                    .onItem()
                    .transformToUni(
                        current -> setInstitutionTypeAndBillingData(current, forceImport))
                    .onItem()
                    .transformToUni(
                        current -> persistOnboarding(onboarding, userRequests, product, null))
                    .onItem()
  ...
  private Uni<Onboarding> setInstitutionTypeAndBillingData(
      Onboarding onboarding, boolean forceImport) {
    if (Objects.nonNull(onboarding.getInstitution())
        && !PSP.equals(onboarding.getInstitution().getInstitutionType())
        && !forceImport) {
      return institutionRegistryProxyApi
          .findInstitutionUsingGET(onboarding.getInstitution().getTaxCode(), null, null)
          .onItem()
          .invoke(
              proxyInstitution -> {
                if (Objects.nonNull(proxyInstitution)) {
                  InstitutionType institutionType =
                      proxyInstitution.getCategory().equalsIgnoreCase(GSP_CATEGORY_INSTITUTION_TYPE)
                          ? InstitutionType.GSP
                          : InstitutionType.PA;
