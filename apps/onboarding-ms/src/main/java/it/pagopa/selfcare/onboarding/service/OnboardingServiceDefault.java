...
public class OnboardingServiceDefault implements OnboardingService {
    ...
    private Uni<Onboarding> addReferencedOnboardingId(Onboarding onboarding) {
        final Map<String, String> queryParameter = QueryUtils.createMapForOnboardingInstitutionQueryParameter(
        ...
        return Onboarding.find(query)
                ...
                .recoverWithUni(Uni.createFrom().failure(new ResourceNotFoundException(String.format(UO_NOT_FOUND.getMessage(), onboarding.getInstitution().getSubunitCode()))))
                .map(Onboarding.class::cast)
                .onItem().invoke(previousOnboarding -> onboarding.setReferencedOnboardingId(previousOnboarding.getId()))
                .replaceWith(onboarding);
    }
