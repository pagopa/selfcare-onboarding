...
class OnboardingUtilsTest {
  ...
  @InjectMock @RestClient UoApi uoApi;

  @InjectMock @RestClient AooApi aooApi;
  ...
  void shouldOnboardingInstitutionWithAdditionalInfo(String type) {
    ...
    OnboardingUtils.ProxyResource proxyResource =
        OnboardingUtils.ProxyResource.builder().type(InstitutionPaSubunitType.EC).build();

    UniAssertSubscriber<Onboarding> subscriber =
        onboardingUtils
  ...
  void checkRecipientCodeNoBilling() {
    ...
    when(aooApi.findByUnicodeUsingGET(any(), any())).thenReturn(Uni.createFrom().item(resource));
  ...
  void checkRecipientCodeFailure() {
    ...
    ClientWebApplicationException exception =
        new ClientWebApplicationException(HttpStatus.SC_INTERNAL_SERVER_ERROR);
  ...
  void checkRecipientCodeNoAssociation() {
    ...
    when(aooApi.findByUnicodeUsingGET(any(), any())).thenReturn(Uni.createFrom().item(aooResource));
