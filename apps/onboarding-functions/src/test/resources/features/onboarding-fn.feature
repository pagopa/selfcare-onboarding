...
Feature: Onboarding collection
  ...
  Scenario Outline: Verify correct invocation of the StartOnboardingOrchestration for correct APPROVE request
    Given Preparing the invocation of "StartOnboardingOrchestration" HTTP call with onboardingId "89ad7142-24bb-48ad-8502-9c9232137e85"
    When I send a GET request with given onboardingId
    Then the response should have status code 202
    And the answer should contain "id,purgeHistoryDeleteUri,restartPostUri"
    And there is a document for onboarding with status "<status>"
  ...
  Scenario Outline: Verify correct invocation of the StartOnboardingOrchestration for correct CONTRACT_REGISTRATION request
