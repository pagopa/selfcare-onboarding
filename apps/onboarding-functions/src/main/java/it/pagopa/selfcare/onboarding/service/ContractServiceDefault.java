...
public class ContractServiceDefault implements ContractService {
    ...
    /**
     * Creates a PDF contract document from a given contract template file and institution data. Based on @contractTemplatePath it loads contract
     * template as test and replace placeholder using a map <key,value> with institution information. Contract will be stored at
    ...
    private File generateAggregatesCsv(
        ...
        switch (productId) {
            ...
            case "prod-pn":
                return new File(
                    String.valueOf(
                        filePath)); // prod-pn is available for aggregator's workflow but csv structure is
            // still not defined.
            default:
                throw new IllegalArgumentException(
