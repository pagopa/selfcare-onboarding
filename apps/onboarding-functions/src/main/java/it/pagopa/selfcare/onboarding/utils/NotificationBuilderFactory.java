...
public class NotificationBuilderFactory {
    ...
    public NotificationBuilderFactory(@ConfigProperty(name = "onboarding-functions.sender-mail") String alternativeEmail) {
    ...
    public NotificationBuilder create(NotificationConfig.Consumer consumer) {
        ...
        if (SC_CONTRACTS_FD.getValue().equalsIgnoreCase(consumer.topic())) {
            notificationBuilder = new FdNotificationBuilder(this.alternativeEmail, consumer, proxyRegistryInstitutionApi, geographicTaxonomiesApi, coreInstitutionApi);
        } else if (SC_CONTRACTS_SAP.getValue().equalsIgnoreCase(consumer.topic())) {
            notificationBuilder = new SapNotificationBuilder(this.alternativeEmail, consumer, proxyRegistryInstitutionApi, geographicTaxonomiesApi, coreInstitutionApi, proxyRegistryUoApi, proxyRegistryAooApi);
        } else if (SC_CONTRACTS.getValue().equalsIgnoreCase(consumer.topic())) {
            notificationBuilder = new StandardNotificationBuilder(this.alternativeEmail, consumer, proxyRegistryInstitutionApi, geographicTaxonomiesApi, coreInstitutionApi);
        } else {
